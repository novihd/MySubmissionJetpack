...
import com.example.mysubmissionjetpack1.helper.JsonHelper
...
class FakeShowRepository(private val remoteDataSource: RemoteDataSource, private val localDataSource: LocalDataSource, private val appExecutors: AppExecutors) : ShowDataSource {
        ...
        return object : NetworkBoundResource<MovieEntity, MovieDetailResponse>(appExecutors) {
            ...
            override fun saveCallResult(data: MovieDetailResponse) {
                val detailMovie = with(data) {
                    MovieEntity(id, poster_path, title, JsonHelper.genreToString(data).toString(), vote_average, overview, release_date, false)
                }
        ...
        return object : NetworkBoundResource<TvEntity, SeriesDetailResponse>(appExecutors) {
            ...
            override fun saveCallResult(data: SeriesDetailResponse) {
                val detailSeries = with(data) {
                    TvEntity(id, poster_path, name, JsonHelper.genreTvToString(data).toString(), vote_average, overview, first_air_date, false)
                }
    ...
    override fun setFavSeries(series: TvEntity, state: Boolean) {
        return localDataSource.setFavSeries(series, state)
    }
